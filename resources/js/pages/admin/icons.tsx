...
export default function AdminIcons({ icons }: AdminIconsProps) {
    const { toast } = useToast();


    const handleDelete = (iconId: number, iconName: string) => {
        if (confirm(`Are you sure you want to delete "${iconName}"? This action cannot be undone.`)) {
            router.delete(`/admin/icons/${iconId}`, {
                preserveScroll: true,
                onSuccess: () => {
                    toast({
                        title: 'Success!',
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={formatTitle('Manage Icons - Admin')} />
                ...
                <div className="flex items-center justify-between bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                    <div>
                        ...
                        <p className="text-gray-600 flex items-center gap-2">
                            <span>Review, approve, and manage all icons on the platform</span>
                            {icons.total > 0 && (
                    ...
                    <Card className="border-gray-200 shadow-lg bg-white">
                        <CardContent className="flex flex-col items-center justify-center py-20">
                            ...
                            <p className="text-gray-600 text-center max-w-md">
                                No icons have been uploaded to the platform yet.
                            </p>
                        </CardContent>
                    </Card>
                ) : (
                        ...
                        {icons.last_page > 1 && (
                            <div className="flex items-center justify-center gap-3 mt-8 bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                                <Button
                                    ...
                                    onClick={() => router.visit(`/admin/icons?page=${icons.current_page - 1}`)}
